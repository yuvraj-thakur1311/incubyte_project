...
describe('Integration Tests', () => {
  it('should handle complete user flow', async () => {
    ...
    const addSweetResponse = await request(app)
      .post('/api/sweets')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({
        name: 'Chocolate Bar',
    ...
    const finalSweetsResponse = await request(app)
      .get('/api/sweets')
      .expect(200);

    const updatedSweet = finalSweetsResponse.body.find((s: any) => s._id === sweetId);
